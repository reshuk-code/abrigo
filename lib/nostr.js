...
export function fetchProfile(pubkeyHex) {
  return new Promise((resolve) => {
    let best = null;
    const open = getOpenRelays();
    if (open.size === 0) return resolve(null);
    let eoseCount = 0;
...
export function fetchRecentGroupInvites(myPubkeyHex, myPrivkeyHex, sinceTs = 0) {
  return new Promise((resolve) => {
    const invites = [], open = getOpenRelays();
    if (open.size === 0) return resolve([]);
    let eoseCount = 0;
    const unsub = subscribe(
