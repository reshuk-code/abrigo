...
const DB_VERSION = 4; // v4: add groups store
...
export async function setDisplayName(name, publish = false) {
  await kvSet('displayName', name.trim());
  if (publish) {
    await publishProfile(name.trim());
  }
}

export async function publishProfile(name) {
  const privkeyHex = await kvGet('privkey');
  if (!privkeyHex) return;
  const { buildProfileEvent, publishEvent } = await import('./nostr');
  const event = buildProfileEvent(name, privkeyHex);
...
export async function clearIdentity() {
  ...
  await new Promise((res, rej) => {
    const tx = db.transaction(['messages', 'requests', 'groups'], 'readwrite');
...
/**
 * group object shape:
 *   { groupId, name, members: [pubkeyHex], groupKeyHex, createdAt, createdBy }
 * groupId  = 'grp_' + random hex
 * groupKeyHex = 32-byte shared symmetric key (hex)
 */

export function generateGroupId() {
  const bytes = new Uint8Array(16);
  crypto.getRandomValues(bytes);
  return 'grp_' + bytesToHex(bytes);
}
